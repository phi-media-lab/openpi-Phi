...
## MI300X / ROCm JAX Notes

This branch also includes MI300X bring-up work that was validated on an AMD ROCm JAX Docker path:

- a `debug`/fake-data training fix that lazily imports `lerobot`
- a reproducible policy benchmark helper at [`scripts/benchmark_policy.py`](scripts/benchmark_policy.py)
- machine-specific bring-up notes at [`docs/mi300x_bringup.md`](docs/mi300x_bringup.md)

Those notes are intentionally kept out of the main installation and usage flow below, since they describe one validated MI300X setup rather than the general upstream path.
